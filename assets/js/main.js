...
const toggleMenuTwo = function () {
	...
	closeDrop.classList.toggle('close');
}
...
document.addEventListener("click", function (e) {
    headerNavMenu.forEach((menu)=> {
        ...
        const its_btnMenu = target == conditionerDrop;
...
const qwizSwiper = new Swiper('.qwiz_wrapper-slider', {
...
qwizSwiper.on("slideChange afterInit init", function () {
    let currentSlide = this.activeIndex + 1;
    document.querySelector('.qwiz-number').innerHTML = `
    <span class="qwiz__current">
    ${currentSlide < 4 ? '' + currentSlide : currentSlide}
    </span> 
    из 
    <span class="qwiz__total">
...
if(qwiz) {
    ...
    qwizSwiper.init();
    const qwizThanks = document.querySelector('.qwiz-thanks');
    const qwizPopupBtn = document.querySelector('.qwiz_popup-wrapper_left-btn');
    const qwizMeasuring = document.querySelector('.qwiz-measuring');
    const qwizWrapperBox = document.querySelector('.qwiz_wrapper-box');
    const qwizNext = document.querySelector('.qwiz-next');
    qwizSwiper.on('slideChange', function() {
        const currentSlideIndex = this.realIndex;
        if(currentSlideIndex === 4) {
            qwizNext.addEventListener('click', () => {
                qwizThanks.classList.add('open');
                qwizWrapperBox.classList.add('none');
            });
        }
    });
    qwizPopupBtn.addEventListener('click', ()=> {
        qwizThanks.classList.add('none');
        qwizMeasuring.classList.add('open');
    })
}
